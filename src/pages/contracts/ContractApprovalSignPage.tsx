...
export default function ContractApprovalSignPage({
  ...
  const [resultContractStatus, setResultContractStatus] = useState<string | null>(null);
  ...
  useEffect(() => {
   ...
   const handleDownload = async () => {
    // Mode eksternal tidak punya endpoint download via contractId
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `${contract?.contract_number || "kontrak"}.pdf`,
      );
  ...
  const handleSubmit = async () => {
    ...
    if (mode === "upload") {
      ...
      if (isExternalMode) {
        const response = await submitExternalContractSignature(token!, signPayload);
        const status = response.contract_status;
        setResultContractStatus(status);
        const msg =
          status === "active"
        ...
        <div className="border-b border-slate-200 bg-slate-50 px-4 py-3 flex items-center justify-between gap-4 sticky top-0 z-20">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-slate-500">
              Pilih metode tanda tangan untuk menyelesaikan persetujuan kontrak
            </p>
          </div>
        </div>
        ...
        <div className="px-4 pb-4 pt-3">
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-800">Download Dokumen</p>
                    <p className="text-xs text-slate-500 mt-0.5">Unduh dokumen kontrak dalam format PDF</p>
                    <button
                      type="button"
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-800">Tanda Tangan Manual</p>
                    <p className="text-xs text-slate-500 mt-0.5">Cetak dokumen, tanda tangan, lalu scan kembali (PDF atau foto)</p>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-slate-800">Upload Dokumen Final</p>
                    <p className="text-xs text-slate-500 mt-0.5">
                      Upload dokumen PDF yang sudah ditandatangani oleh kedua pihak. 
                      Dokumen ini akan dikirimkan ke pihak eksternal untuk dikonfirmasi.
                    </p>
                    <div className="mt-2 rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-5 text-center">
                      ...
                      {/* ✅ Hanya PDF */}
                      <p className="text-xs text-slate-500">Format: PDF (maks. 20MB)</p>
                      {uploadFile && (
            ...
            <div className="mt-4 rounded-lg border border-sky-100 bg-sky-50 px-4 py-3 text-xs leading-5 text-sky-700">
              {mode === "upload" ? (
                <>
                  <strong>Catatan:</strong> Dokumen yang diupload adalah dokumen final yang sudah 
                  ditandatangani kedua pihak secara langsung. Setelah diupload, pihak eksternal 
                  akan menerima email konfirmasi.
                </>
              ) : (
                <>
                  <strong>Catatan:</strong> Tanda tangan digital memiliki kekuatan hukum yang sama 
                  dengan tanda tangan fisik. Dengan menandatangani, Anda menyetujui isi kontrak ini.
                </>
              )}
            </div>
          </div>
