...
export default function ContractEditorPage() {
  ...
  const draftKey = id
    ? `contract_draft_${id}`
    : renewFromId
      ? `contract_draft_renew_${renewFromId}`
      : initialTemplate
        ? `contract_draft_template_${initialTemplate.id}`
        : "contract_draft_new";
  ...
  const [parentContractId, setParentContractId] = useState<number | null>(
    renewFromId,
  );
  ...
  useEffect(() => {
    ...
    // Nomor kontrak digenerate sekali saat kontrak baru dibuat agar identitas dokumen
    // sudah tersedia sebelum HRD menyimpan draft atau mengajukan kontrak.
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const source = (await fetchContract(renewFromId)) as ContractDetail;
        const sourceContent = source.content ?? "";
        const extractedMargins = extractMarginsFromContent(sourceContent);
        const extractedContent = extractWatermarkFromContent(
          extractedMargins.content,
        );
        setParentContractId(source.id);
        setExternalContractNumber("");
        ...
        setSelectedPartnerName(
          source.partner && source.partner !== "-" ? source.partner : "",
        );
        setPaperSize(normalizePaperSize(source.paper_size));
        setWatermark(extractedContent.watermark);
        setPageMargin(extractedMargins.margins || DEFAULT_MARGINS);
        ...
        if (source.template_id) {
          setSelectedTemplate({
            ...
            paper_size: normalizePaperSize(source.paper_size),
          } as TemplateOption);
        } else {
          setEditorContentWithoutHistory(
            editor,
        ...
        if (source.signers?.length) {
          setSigners(
            source.signers.map((signer, index) => ({
              id: `renew-signer-${index}`,
              type: signer.signer_type,
              name:
                signer.signer_type === "internal" && signer.user
                  ? (signer.user.name ?? "")
                  : (signer.signer_name ?? ""),
              title:
                signer.signer_type === "internal" && signer.user
        ...
        renewalLoadedRef.current = false;
        setSaveError(
          getApiErrorMessage(
            error,
            "Gagal memuat kontrak asal untuk diperpanjang.",
          ),
        );
      }
    })();
  }, [renewFromId, editor, fieldsLoaded, allFields]);
  ...
  const getSubmitSignerValidationMessage = () => {
    // Validasi signer hanya berlaku saat Ajukan, bukan saat Simpan Draft,
    // supaya HRD tetap bisa menyimpan progres yang belum lengkap.
  ...
  const getSubmitFieldValidationMessage = () => {
    // Field wajib dicek dari token yang masih tertanam di HTML editor.
    // Ini memastikan field dinamis tidak hanya tampil visual, tetapi benar-benar terisi.
  ...
  // Content, watermark, margin, field_values, signer, dan mitra dikirim dalam satu payload.
  const buildPayload = (
    statusOverride?: string,
    overrides?: Partial<{ title: string }>,
  ) => {
    const rawContent = editor?.getHTML() ?? "";
    // Marker watermark dan margin ikut disimpan di content agar PDF backend
    // dapat merender dokumen sesuai pengaturan editor.
  ...
  // Dengan cara ini tab Preview tetap bisa dibuat tanpa memaksa semua data pengajuan terisi.
